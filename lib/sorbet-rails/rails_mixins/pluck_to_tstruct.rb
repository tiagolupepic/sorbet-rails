...
module SorbetRails::PluckToTStruct
  ...
  def pluck_to_tstruct(ta_struct, associations: {})
    ...
    pluck(*pluck_keys).map do |row|
      row = [row] if keys_one
      value = Hash[map_enums_nil_values_and_defaults(tstruct_props, tstruct_keys.zip(row))]
      tstruct.new(value)
    end
  end
  ...
  sig {params(type_object: T::Types::Base).returns(T::Boolean)}
  private def type_enum?(type_object)
    if type_object.is_a?(T::Types::Union)
      return type_object.types.any? { |type| type.raw_type.ancestors.include?(T::Enum) }
    end

    type_object.raw_type.ancestors.include?(T::Enum)
  end
  ...
  private def map_enums_nil_values_and_defaults(tstruct_props, zipped_rows)
    ...
    zipped_rows.map do |key, value|
      ...
      if type_enum?(type_object)
        if nilable?(type_object)
          next [key, type_object.types.first.raw_type.try_deserialize(value) || default]
        end

        next [key, type_object.raw_type.deserialize(value)]
      end
